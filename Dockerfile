...
RUN echo '' && \
    ...
    echo '================================' && \
    echo '' && \
    apt-get install -y --no-install-recommends sudo bash-completion build-essential openssl libssl-dev libpcre3 libpcre3-dev zlib1g-dev \
    net-tools iputils-ping nmap lsof telnet cron rsyslog expect zip unzip p7zip-full curl wget locales git zsh gosu apt-transport-https \
